"use client";
// app/page.tsx — Javari Action Archive · CR AudioViz AI · EIN 39-3646201 · May 2026
import { useState } from "react";
const T=[{"i": "\ud83d\udcb0", "l": "Figure Value", "d": "Action figure valuations", "h": "/value"}, {"i": "\ud83d\udccb", "l": "Collection", "d": "Inventory management", "h": "/collection"}, {"i": "\ud83d\udd0d", "l": "Identifier", "d": "Identify rare figures", "h": "/identify"}, {"i": "\u270d\ufe0f", "l": "Listing Writer", "d": "eBay auction descriptions", "h": "/listing"}];
export default function P() {
  ...
  async function go() { if(!i.trim())return; setL(true);setO("");
    try { const r=await fetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",content:i}],stream:false,systemOverride:"You are an action figure expert and collector."})});
  ...
  return (<div style={{minHeight:"100vh",background:"#040912",color:"#e2e8f0",fontFamily:"system-ui"}}>
    <nav style={{background:"#1E3A5F",padding:"0 20px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
      <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:20}}>📦</span><span style={{fontWeight:800,color:"#FF0800",fontSize:15}}>Javari Action Archive</span></div>
    ...
    <section style={{background:"linear-gradient(135deg,#1E3A5F,#040912)",padding:"48px 24px 40px",textAlign:"center"}}>
      <h1 style={{fontSize:"clamp(22px,4vw,42px)",fontWeight:900,color:"#fff",margin:"0 0 10px",lineHeight:1.05}}>Action Figure<br/><span style={{color:"#FF0800"}}>Collection Hub</span></h1>
      <p style={{color:"rgba(255,255,255,0.7)",fontSize:15,margin:0}}>Track, value, and manage your action figure collection.</p>
    </section>
    <section style={{maxWidth:700,margin:"0 auto",padding:"24px 20px 0"}}>
      <div style={{background:"#0F1F32",border:"1px solid rgba(0,180,216,0.12)",borderRadius:14,padding:"18px 22px"}}>
        <div style={{display:"flex",gap:8}}><input value={i} onChange={e=>setI(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Value this action figure: 1977 Kenner Star Wars Luke Skywalker 12-back AFA 85" style={{flex:1,background:"#172D48",border:"1px solid rgba(0,180,216,0.15)",borderRadius:8,padding:"10px 12px",color:"#e2e8f0",fontSize:13,outline:"none",fontFamily:"system-ui"}}/>
